...
SCENETYPEMODEL_ONE_OF_SCHEMAS = ["FPSFile", "Manual"]
...
class SceneTypeModel(BaseModel):
    ...
    """
    # data type: Manual
    oneof_schema_1_validator: Optional[Manual] = None
    # data type: FPSFile
    oneof_schema_2_validator: Optional[FPSFile] = None
    actual_instance: Optional[Union[FPSFile, Manual]] = None
    one_of_schemas: Set[str] = { "FPSFile", "Manual" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SceneTypeModel.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Manual):
            ...
            match += 1
        # validate data type: FPSFile
        if not isinstance(v, FPSFile):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FPSFile`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SceneTypeModel with oneOf schemas: FPSFile, Manual. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SceneTypeModel with oneOf schemas: FPSFile, Manual. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into FPSFile
        try:
            instance.actual_instance = FPSFile.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SceneTypeModel with oneOf schemas: FPSFile, Manual. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SceneTypeModel with oneOf schemas: FPSFile, Manual. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], FPSFile, Manual]]:
