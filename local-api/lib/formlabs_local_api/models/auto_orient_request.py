...
AUTOORIENTREQUEST_ONE_OF_SCHEMAS = ["Default", "DentalMode"]
...
class AutoOrientRequest(BaseModel):
    ...
    # data type: DentalMode
    oneof_schema_2_validator: Optional[DentalMode] = None
    actual_instance: Optional[Union[Default, DentalMode]] = None
    one_of_schemas: Set[str] = { "Default", "DentalMode" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AutoOrientRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Default):
            ...
            match += 1
        # validate data type: DentalMode
        if not isinstance(v, DentalMode):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DentalMode`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AutoOrientRequest with oneOf schemas: Default, DentalMode. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AutoOrientRequest with oneOf schemas: Default, DentalMode. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into DentalMode
        try:
            instance.actual_instance = DentalMode.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AutoOrientRequest with oneOf schemas: Default, DentalMode. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AutoOrientRequest with oneOf schemas: Default, DentalMode. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Default, DentalMode]]:
