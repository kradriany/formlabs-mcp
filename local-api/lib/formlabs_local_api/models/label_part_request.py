...
class LabelPartRequest(BaseModel):
    """
    LabelPartRequest
    """ # noqa: E501
    model_id: StrictStr = Field(description="The ID of the model to label")
    orientation: OrientationModel
    position: ScenePositionModel
    label: StrictStr = Field(description="The label's text")
    application_mode: Optional[StrictStr] = 'EMBOSS'
    font_size_mm: Union[Annotated[float, Field(le=20, strict=True, ge=2)], Annotated[int, Field(le=20, strict=True, ge=2)]] = Field(description="The font size of the label")
    depth_mm: Union[Annotated[float, Field(le=10, strict=True, ge=0.2)], Annotated[int, Field(le=10, strict=True, ge=1)]] = Field(description="The label's extrusion depth. This depth is a constant offset from the model's surface.")
    __properties: ClassVar[List[str]] = ["model_id", "orientation", "position", "label", "application_mode", "font_size_mm", "depth_mm"]
    ...
    def application_mode_validate_enum(cls, value):
        ...
        if value not in set(['EMBOSS', 'ENGRAVE']):
            raise ValueError("must be one of enum values ('EMBOSS', 'ENGRAVE')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "label": obj.get("label"),
            "application_mode": obj.get("application_mode") if obj.get("application_mode") is not None else 'EMBOSS',
            "font_size_mm": obj.get("font_size_mm"),
