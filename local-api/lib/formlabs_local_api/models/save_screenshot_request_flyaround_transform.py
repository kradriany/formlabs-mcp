...
class SaveScreenshotRequestFlyaroundTransform(BaseModel):
    """
    SaveScreenshotRequestFlyaroundTransform
    """ # noqa: E501
    transform: Optional[Annotated[List[SaveScreenshotRequestFlyaroundTransformTransformInner], Field(min_length=2)]] = None
    frame_count: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The desired number of frames in the animation.")
    ensure_constant_size: Optional[StrictBool] = Field(default=None, description="Whether or not to ensure that every frame in the animation is the same size.")
    __properties: ClassVar[List[str]] = ["transform", "frame_count", "ensure_constant_size"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "transform": [SaveScreenshotRequestFlyaroundTransformTransformInner.from_dict(_item) for _item in obj["transform"]] if obj.get("transform") is not None else None,
            "frame_count": obj.get("frame_count"),
            "ensure_constant_size": obj.get("ensure_constant_size")
        })
        return _obj
