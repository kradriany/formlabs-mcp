...
class ListMaterials200Response(BaseModel):
    """
    ListMaterials200Response
    """ # noqa: E501
    printer_types: Optional[List[ListMaterials200ResponsePrinterTypesInner]] = None
    __properties: ClassVar[List[str]] = ["printer_types"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in printer_types (list)
        _items = []
        if self.printer_types:
            for _item_printer_types in self.printer_types:
                if _item_printer_types:
                    _items.append(_item_printer_types.to_dict())
            _dict['printer_types'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "printer_types": [ListMaterials200ResponsePrinterTypesInner.from_dict(_item) for _item in obj["printer_types"]] if obj.get("printer_types") is not None else None
        })
        return _obj
