...
class HollowModelRequest(BaseModel):
    """
    HollowModelRequest
    """ # noqa: E501
    models: Optional[ModelsSelectionModel] = None
    wall_thickness_mm: Optional[Union[Annotated[float, Field(strict=True, ge=0.1)], Annotated[int, Field(strict=True, ge=1)]]] = Field(default=None, description="The desired wall thickness of the hollowed models")
    feature_size_mm: Optional[Union[Annotated[float, Field(strict=True, ge=0.05)], Annotated[int, Field(strict=True, ge=1)]]] = Field(default=1, description="The size of features to keep on hollowed geometry. Higher values will produce a smoother, simpler inner hollowed surface with a less accurate thickness wall thickness around features smaller than this value.")
    accuracy: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=0.5, description="The accuracy of the hollowing operation. Higher values are slower but more accurate.")
    __properties: ClassVar[List[str]] = ["models", "wall_thickness_mm", "feature_size_mm", "accuracy"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "feature_size_mm": obj.get("feature_size_mm") if obj.get("feature_size_mm") is not None else 1,
            "accuracy": obj.get("accuracy") if obj.get("accuracy") is not None else 0.5
        })
        return _obj
