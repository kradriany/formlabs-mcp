...
ESTIMATEDPRINTTIMEMODEL_ONE_OF_SCHEMAS = ["SLAPrinterTypes", "SLSPrinterTypes"]

class EstimatedPrintTimeModel(BaseModel):
    """
    EstimatedPrintTimeModel
    """
    # data type: SLAPrinterTypes
    oneof_schema_1_validator: Optional[SLAPrinterTypes] = None
    # data type: SLSPrinterTypes
    oneof_schema_2_validator: Optional[SLSPrinterTypes] = None
    actual_instance: Optional[Union[SLAPrinterTypes, SLSPrinterTypes]] = None
    one_of_schemas: Set[str] = { "SLAPrinterTypes", "SLSPrinterTypes" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = EstimatedPrintTimeModel.model_construct()
        error_messages = []
        match = 0
        # validate data type: SLAPrinterTypes
        if not isinstance(v, SLAPrinterTypes):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SLAPrinterTypes`")
        else:
            match += 1
        # validate data type: SLSPrinterTypes
        if not isinstance(v, SLSPrinterTypes):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SLSPrinterTypes`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in EstimatedPrintTimeModel with oneOf schemas: SLAPrinterTypes, SLSPrinterTypes. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in EstimatedPrintTimeModel with oneOf schemas: SLAPrinterTypes, SLSPrinterTypes. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SLAPrinterTypes
        try:
            instance.actual_instance = SLAPrinterTypes.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into SLSPrinterTypes
        try:
            instance.actual_instance = SLSPrinterTypes.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into EstimatedPrintTimeModel with oneOf schemas: SLAPrinterTypes, SLSPrinterTypes. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EstimatedPrintTimeModel with oneOf schemas: SLAPrinterTypes, SLSPrinterTypes. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], SLAPrinterTypes, SLSPrinterTypes]]:
