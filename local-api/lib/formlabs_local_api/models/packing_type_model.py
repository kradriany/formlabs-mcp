...
class PackingTypeModel(BaseModel):
    """
    Different ways to pack a selection of models
    """ # noqa: E501
    packing_type: Optional[StrictStr] = Field(default='PACK_VOLUME', description="The optimization strategy used for packing. Minimize the height or volume of the resulting cage, or skip packing entirely")
    __properties: ClassVar[List[str]] = ["packing_type"]

    @field_validator('packing_type')
    def packing_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['PACK_NORMAL', 'PACK_VOLUME', 'PACK_HEIGHT', 'PACK_NONE']):
            raise ValueError("must be one of enum values ('PACK_NORMAL', 'PACK_VOLUME', 'PACK_HEIGHT', 'PACK_NONE')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "packing_type": obj.get("packing_type") if obj.get("packing_type") is not None else 'PACK_VOLUME'
        })
        return _obj
