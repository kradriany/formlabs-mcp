...
class SaveScreenshotRequestFlyaroundTransformTransformInner(BaseModel):
    """
    SaveScreenshotRequestFlyaroundTransformTransformInner
    """ # noqa: E501
    yaw: Union[StrictFloat, StrictInt] = Field(description="Yaw rotation in degrees for the camera's view, where 0º looks down the negative X-axis")
    pitch: Union[StrictFloat, StrictInt] = Field(description="Pitch rotation in degrees for the camera's view, where 0º looks flat from the horizon and positive angles look down on models (in SLA scenes, toward the build platform)")
    __properties: ClassVar[List[str]] = ["yaw", "pitch"]
