...
LABELPART400RESPONSEERRORMESSAGE_ONE_OF_SCHEMAS = ["str"]

class LabelPart400ResponseErrorMessage(BaseModel):
    """
    LabelPart400ResponseErrorMessage
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="Model not found")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(default=None, description="Failed to create label for part <error message>")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = LabelPart400ResponseErrorMessage.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in LabelPart400ResponseErrorMessage with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in LabelPart400ResponseErrorMessage with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into LabelPart400ResponseErrorMessage with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into LabelPart400ResponseErrorMessage with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance
