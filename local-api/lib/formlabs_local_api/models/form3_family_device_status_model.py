...
class Form3FamilyDeviceStatusModel(BaseModel):
    """
    ...
    cartridge_data: Dict[str, Form4FamilyDeviceStatusModelAllOfCartridgeData]
    form_auto_status: StrictStr
    __properties: ClassVar[List[str]] = ["id", "product_name", "status", "is_connected", "connection_type", "ip_address", "firmware_version", "ready_to_print_now", "is_remote_print_enabled", "estimated_print_time_remaining_ms", "tank_id", "tank_material_code", "cartridge_data", "form_auto_status"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "cartridge_data": dict(
                (_k, Form4FamilyDeviceStatusModelAllOfCartridgeData.from_dict(_v))
                for _k, _v in obj["cartridge_data"].items()
            )
            if obj.get("cartridge_data") is not None
            else None,
            "form_auto_status": obj.get("form_auto_status")
        })
        return _obj
