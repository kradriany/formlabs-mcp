...
class PrintRequest(BaseModel):
    ...
    PrintRequest
    """ # noqa: E501
    printer: StrictStr = Field(description="Printer serial name, IP address, or Fleet Control Queue ID")
    job_name: StrictStr
    find_printer_timeout_seconds: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=30, description="Number of seconds to wait to find the given printer.")
    print_now: Optional[StrictBool] = Field(default=None, description="If true, the job should be printed immediately if the printer's \"ready_to_print_now\" status is true. Attempting to print now on a printer that does not support it will result in an error. If false, the job should be uploaded to the printer's local queue. Not including this value will default to printing now if the printer is in a ready state, and queueing the job otherwise. ")
    __properties: ClassVar[List[str]] = ["printer", "job_name", "find_printer_timeout_seconds", "print_now"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "find_printer_timeout_seconds": obj.get("find_printer_timeout_seconds") if obj.get("find_printer_timeout_seconds") is not None else 30,
            "print_now": obj.get("print_now")
        })
        return _obj
