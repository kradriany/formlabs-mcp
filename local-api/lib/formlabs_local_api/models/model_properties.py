...
class ModelProperties(BaseModel):
    ...
    bounding_box: Optional[ModelPropertiesBoundingBox] = None
    ...
    canonical_model_hash: Optional[StrictStr] = Field(default=None, description="The hash of the mesh data and its transform (position, orientation, and scale)")
    lock: Optional[LockModel] = LockModel.FREE
    __properties: ClassVar[List[str]] = ["id", "name", "position", "orientation", "scale", "units", "bounding_box", "original_file", "visible", "has_supports", "in_bounds", "raw_mesh_hash", "canonical_model_hash", "lock"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "scale": obj.get("scale"),
            "units": obj.get("units"),
            "bounding_box": ModelPropertiesBoundingBox.from_dict(obj["bounding_box"]) if obj.get("bounding_box") is not None else None,
            ...
            "lock": obj.get("lock") if obj.get("lock") is not None else LockModel.FREE
        })
        return _obj
