...
class PrintValidationResultModel(BaseModel):
    """
    PrintValidationResultModel
    """ # noqa: E501
    per_model_results: Optional[Dict[str, PrintValidationResultModelPerModelResultsValue]] = Field(default=None, description="A map of model IDs to their print validation results.")
    __properties: ClassVar[List[str]] = ["per_model_results"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in per_model_results (dict)
        _field_dict = {}
        if self.per_model_results:
            for _key_per_model_results in self.per_model_results:
                if self.per_model_results[_key_per_model_results]:
                    _field_dict[_key_per_model_results] = self.per_model_results[_key_per_model_results].to_dict()
            _dict['per_model_results'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "per_model_results": dict(
                (_k, PrintValidationResultModelPerModelResultsValue.from_dict(_v))
                for _k, _v in obj["per_model_results"].items()
            )
            if obj.get("per_model_results") is not None
            else None
        })
        return _obj
