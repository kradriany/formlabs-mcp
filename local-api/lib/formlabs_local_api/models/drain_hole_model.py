...
class DrainHoleModel(BaseModel):
    """
    A drain hole of the model in the scene. Composes OrientationModel and ScenePositionModel
    """ # noqa: E501
    orientation: OrientationModel
    position: ScenePositionModel
    diameter_mm: Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]] = Field(description="The diameter of the drain hole, in mm")
    depth_mm: DrainHoleModelDepthMm
    outset_mm: Optional[Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]]] = Field(default=None, description="The back depth of the drain hole, in mm")
    create_plug: StrictBool = Field(description="Whether or not to create a plug for the drain hole")
    max_search_distance: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The maximum distance, in mm, from the given position to search for a point on the surface of the model. Must be combined with depth_mm: \"AUTO\".")
    __properties: ClassVar[List[str]] = ["orientation", "position", "diameter_mm", "depth_mm", "outset_mm", "create_plug", "max_search_distance"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of depth_mm
        if self.depth_mm:
            _dict['depth_mm'] = self.depth_mm.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "depth_mm": DrainHoleModelDepthMm.from_dict(obj["depth_mm"]) if obj.get("depth_mm") is not None else None,
            "outset_mm": obj.get("outset_mm"),
            "create_plug": obj.get("create_plug"),
            "max_search_distance": obj.get("max_search_distance")
        })
        return _obj
