...
class AutoSupportRequest(BaseModel):
    """
    AutoSupportRequest
    """ # noqa: E501
    models: Optional[ModelsSelectionModel] = None
    density: Optional[Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]]] = Field(default=None, description="Unitless factor to adjust the density of supports (default is 1.0)")
    slope_multiplier: Optional[Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]]] = Field(default=None, description="Unitless factor to increase or decrease support density on steep slopes (default is 1.0)")
    only_minima: Optional[StrictBool] = Field(default=None, description="Whether to only generate touchpoints on local minima, skipping all other supports. Default is false.")
    ...
    __properties: ClassVar[List[str]] = ["models", "density", "slope_multiplier", "only_minima", "raft_type", "raft_label_enabled", "breakaway_structure_enabled", "touchpoint_size_mm", "internal_supports_enabled", "raft_thickness_mm", "height_above_raft_mm", "z_compression_correction_mm", "early_layer_merge_mm"]
    ...
    def raft_type_validate_enum(cls, value):
        ...
        if value not in set(['FULL_RAFT', 'MINI_RAFTS', 'MINI_RAFTS_ON_BP']):
            raise ValueError("must be one of enum values ('FULL_RAFT', 'MINI_RAFTS', 'MINI_RAFTS_ON_BP')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "only_minima": obj.get("only_minima"),
