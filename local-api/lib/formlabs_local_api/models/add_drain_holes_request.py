...
class AddDrainHolesRequest(BaseModel):
    """
    AddDrainHolesRequest
    """ # noqa: E501
    model_id: StrictStr = Field(description="The ID of the model to add drain holes to")
    drain_holes: Annotated[List[DrainHoleModel], Field(min_length=1)] = Field(description="List of drain holes to add")
    __properties: ClassVar[List[str]] = ["model_id", "drain_holes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in drain_holes (list)
        _items = []
        if self.drain_holes:
            for _item_drain_holes in self.drain_holes:
                if _item_drain_holes:
                    _items.append(_item_drain_holes.to_dict())
            _dict['drain_holes'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "drain_holes": [DrainHoleModel.from_dict(_item) for _item in obj["drain_holes"]] if obj.get("drain_holes") is not None else None
        })
        return _obj
