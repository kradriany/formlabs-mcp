...
class AutoLayoutRequest(BaseModel):
    ...
    mode: Optional[StrictStr] = Field(default=None, description="`\"DENTAL\"` mode applies algorithms used in PreForm's Dental Workspace. Leave mode unset to use algorithms from the Standard Workspace.")
    ...
    custom_bounds: Optional[AutoLayoutRequestCustomBounds] = None
    __properties: ClassVar[List[str]] = ["models", "mode", "model_spacing_mm", "allow_overlapping_supports", "lock_rotation", "build_platform_2_optimized", "custom_bounds"]
    ...
    def mode_validate_enum(cls, value):
        ...
        if value not in set(['DENTAL']):
            raise ValueError("must be one of enum values ('DENTAL')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of custom_bounds
        if self.custom_bounds:
            _dict['custom_bounds'] = self.custom_bounds.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "custom_bounds": AutoLayoutRequestCustomBounds.from_dict(obj["custom_bounds"]) if obj.get("custom_bounds") is not None else None
        })
        return _obj
